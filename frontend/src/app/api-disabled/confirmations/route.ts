...
export async function PATCH(req: Request) {
  try {
    ...
    if (isCommissionApproval) {
      // Use a transaction to update both the confirmation request and employee status
      const result = await db.$transaction(async (tx) => {
        // Update the confirmation request
        const updatedRequest = await tx.confirmationRequest.update({
          where: { id },
          data: updateData,
          include: {
            employee: true
        ...
        // Update the employee status to "Confirmed" and set confirmation date
      ...
      // Regular update without employee status change
