  ...
  private async request<T>(
    ...
    try {
      ...
      if (response.status === 401) {
        ...
        if (typeof window !== 'undefined') {
          this.clearToken();
          // Temporarily disable redirect to see which API calls are failing
          console.error('Would redirect to login due to 401 on:', endpoint);
      ...
      if (contentType && contentType.includes('application/json')) {
        ...
        console.log('API Client - Raw JSON response:', data);
      } else {
        data = await response.text();
        console.log('API Client - Raw text response:', data);
      }

      if (!response.ok) {
        console.log('API Client - Response not OK:', response.status, response.statusText);
        return {
          success: false,
      ...
      console.log('API Client - Returning success response:', { success: true, data });
      return {
        success: true,
        data,
      };
