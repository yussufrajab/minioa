...
import com.zanzibar.csms.validation.ValidCadreChangeRequest;
...
@AllArgsConstructor
@Builder
@ValidCadreChangeRequest
public class CadreChangeRequestCreateDto {

    @NotBlank(message = "Employee ID is required")
    private String employeeId;

    @NotBlank(message = "Current cadre is required")
    @Size(max = 100, message = "Current cadre cannot exceed 100 characters")
    private String currentCadre;

    @NotBlank(message = "Proposed cadre is required")
    ...
    @NotNull(message = "Education completion year is required")
    ...
    public boolean isValidEducationYear() {
        if (educationCompletionYear == null) {
            return false;
        }
        int currentYear = LocalDate.now().getYear();
        return educationCompletionYear >= 1900 && educationCompletionYear <= currentYear;
    }
